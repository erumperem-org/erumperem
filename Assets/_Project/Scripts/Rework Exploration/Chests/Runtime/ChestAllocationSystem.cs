...
namespace Core.Chests
{
    ...
    public sealed class ChestAllocationSystem : MonoBehaviour
    {
        ...
        public struct ChestCountRange
        {
            ...
            /// <summary>
            /// Rolls a value within [Min, Max], inclusive.
            /// Clamped defensively in case Min > Max.
            /// </summary>
            public int Resolve()
            {
        ...
        private ICorruptionTierSource TierSource =>
            _corruptionTierSource as ICorruptionTierSource;

        /// <summary>
        /// Resolves the tier to use: the real ICorruptionTierSource when assigned,
        /// otherwise the manual test tier (defaults to 0).
        /// </summary>
        public int CurrentTier =>
            TierSource?.CurrentTier ?? _manualTestTier;
        ...
        public async void Initialize()
        {
            if (_poolCreated)
            {
                Log(
                    LogLevel.Warning,
                    "Initialize called but the pool already exists — ignored."
                );
            ...
            // synchronously within the same Start() call.
            await Task.Yield();
        ...
        /// <summary>
        /// Entry point for the external event that requests repositioning
        /// of the already-existing chests. Each call re-rolls the active
        /// count within the current tier's range.
        /// </summary>
        public void Reallocate()
        {
            if (!_poolCreated)
            {
                Log(
                    LogLevel.Warning,
                    "Reallocate called before pool creation — call Initialize() first."
                );
        ...
        private async Task CreatePoolOnceAsync()
        {
            ...
            var initialPositions =
                PickRandomSubset(_allPositions, maxPoolSize);

            AllocationResult result =
                await Allocator.AllocateObjectsAsync(
                    _chestPool,
                    initialPositions,
                    _instancesParent
                );
            ...
            foreach (var placed in result.PlacedObjects)
            {
                ...
                if (chest == null)
                {
                    Log(
                        LogLevel.Error,
                        $"Prefab '{placed.SourceData.name}' has no Chest component."
                    );
            ...
            Log(
                LogLevel.Debug,
                $"Chest pool created: {_chestPoolInstances.Count} instance(s)."
            );
        }

        /// <summary>
        /// The pool must be large enough to cover the highest possible roll
        /// across every tier's range.
        /// </summary>
        ...
        private void ApplyTierBasedPositions()
        {
            ...
            int desiredCount =
                ResolveCountForTier(tier);

            var chosenPositions =
                PickRandomSubset(_allPositions, desiredCount);

            var activeChests =
                new List<Chest>(desiredCount);
            ...
            for (int i = 0; i < _chestPoolInstances.Count; i++)
            {
                ...
                // Only the position is updated here.
                // Rotation remains the random rotation originally rolled
                // by SceneObjectAllocationSystem during pool creation.
            ...
            Log(
                LogLevel.Debug,
                $"Chests repositioned: {desiredCount} active at tier {tier}."
            );
        ...
        private int ResolveCountForTier(int tier)
        {
            if (
                tier < 0 ||
                tier >= _chestCountRangeByTier.Length
            )
            {
                Log(
                    LogLevel.Warning,
                    $"Tier {tier} outside the count range lookup — using 0."
                );
            ...
            int rolled =
                _chestCountRangeByTier[tier].Resolve();

            return Mathf.Min(
                rolled,
                _chestPoolInstances.Count
            );
        }

        private List<Transform> PickRandomSubset(
            List<Transform> source,
            int count)
        {
            var pool =
                new List<Transform>(source);
        ...
        {
            if (_objectAllocationSystem == null)
            {
                Log(
                    LogLevel.Error,
                    "SceneObjectAllocationSystem not assigned."
                );
            ...
            if (
                _chestCountRangeByTier == null ||
                _chestCountRangeByTier.Length == 0)
            {
                Log(
                    LogLevel.Error,
                    "_chestCountRangeByTier is empty."
                );
        ...
        private void Log(
            ...
            LoggerService.PrintLogMessage(
                level,
                $"[ChestAllocationSystem:{gameObject.name}] {msg}",
